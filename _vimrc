...
noremap <silent> \fh :silent %s/^.\{-}\ze\%(POST\\|GET\)//e \| silent %s/$/\r\r\r/e \| silent %s/\\r\\n/\r/e \| silent exe "g/^\\%(GET\\\\|POST\\)/norm 0]u$0]u$0]u$" \| silent exe "g//s/.*\\zs\\zeHTTP\\//\\r  /e" \| silent exe "g/^\\%(GET\\\\|POST\\)/s/\\ze[?&;]\\+/\\r    /e" \| silent exe "g/^Cookie:/s/\\%(^Cookie\\zs:\\\\|;\\) \\+/\\r    /e" \| silent %s/\n\+\%$//e \| silent %s/\s\+$//e \| silent let @q='V/^\%(Cookie\\|$\)\\|\%$/?\S\+::Align! =p0P0 ^[^:]\+:\s\+' \| silent exe "g/^User-Agent:/norm @q" \| set nowrap \| silent exe "nohlsearch" \| silent norm gg<CR>
abb teh the
abb fo of
abb taht that
abb wehn when
